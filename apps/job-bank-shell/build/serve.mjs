...
const outputPath = 'dist/apps/job-bank-shell/browser';
const port = Number(process.env.JOB_BANK_SHELL_DEV_PORT ?? 4205);

await rm(outputPath, { recursive: true, force: true });
await mkdir(outputPath, { recursive: true });
await cp('apps/job-bank-shell/public', outputPath, { recursive: true });
await cp('apps/job-bank-shell/src/index.html', join(outputPath, 'index.html'));
...
await runEsBuildBuilder('apps/job-bank-shell/federation.config.mjs', {
  workspaceRoot: process.cwd(),
  outputPath,
  tsConfig: 'apps/job-bank-shell/tsconfig.app.json',
...
const mainCtx = await esbuild.context({
  entryPoints: ['apps/job-bank-shell/src/main.tsx'],
  outdir: outputPath,
  bundle: true,
  splitting: true,
  format: 'esm',
...
createServer(async (req, res) => {
  ...
  console.log(`job-bank-shell dev server listening on http://localhost:${port}`);
});
