...
describe('LoginPage', () => {
  ...
  it('stays put when there is no active session', () => {
    renderLoginPage();
    expect(screen.queryByText('Job Bank page')).not.toBeInTheDocument();
  });

  it('redirects to job-bank when a session already exists', async () => {
  ...
  it("signIn redirects to mock-idp's /authorize with a PKCE code challenge", async () => {
    ...
    expect(url.searchParams.get('client_id')).toBe('mfe-pot-job-bank-shell');
