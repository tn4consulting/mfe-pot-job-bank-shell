 ...
 * `main.tsx`'s own comment, and `mfe-pot-platform/CLAUDE.md`'s
 * "Bare-specifier workspace libraries can't be imported from an app's own
 * entry" section). Mirrors `auth-flight.ts`'s existing `crypto.subtle`
 * usage for PKCE's SHA-256 challenge.
 *
 * This duplicates verification logic that also exists (jose-backed) in
 * `shared-remote-integrity` -- an accepted tradeoff, the same shape
 * `main.tsx`'s own `resolveFederationManifest` already accepts by
 * hand-inlining `shared-remote-registry`'s fuller logic. Both copies are
 * tested against the same fixture keypair/JWS to catch drift -- see this
 * file's own `.spec.ts` and `shared-remote-integrity`'s
 * `verify-remote-manifest.spec.ts`.
 *
