 ...
 * unknown> | undefined`) -- `apps/job-bank-shell/src/runtime-config.ts` (used by
 ...
 * to consume, so Stage B never needs to re-fetch/re-verify
 * `remoteEntry.json` itself, only the one exposed chunk about to execute.
 *
 ...
 * and admits every candidate as-is -- `nx serve` never signs anything, so
 * strict enforcement would make local dev permanently unable to load any
 * remote. Still warns loudly per remote so an insecure dev session is
 * never silent about it; never blocks, since blocking isn't this flag's
 * job -- see the type's own doc comment on `ShellRuntimeConfig` above.
 */
