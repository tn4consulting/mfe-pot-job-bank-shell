#!/usr/bin/env bash
# Builds job-bank-shell's image, spins up (or reuses) a local kind cluster with
...
HOSTNAME=job-bank-shell.mfe-pot.local
...
echo "==> Building job-bank-shell image..."
docker build \
  --secret id=npm_token,src="$token_file" \
  -t mfe-pot-job-bank-shell:kind \
  -f apps/job-bank-shell/Dockerfile .

echo "==> Loading image into kind..."
kind load docker-image mfe-pot-job-bank-shell:kind --name "$CLUSTER_NAME"
...
echo "==> job-bank-shell is up: curl -H \"Host: $HOSTNAME\" http://localhost/"
